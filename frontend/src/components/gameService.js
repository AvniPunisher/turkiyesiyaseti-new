...
const gameService = {
      ...
      if (saveId) {
        ...
        if (response.success && response.data.savedGames?.length > 0) {
          ...
          const autoSaves = response.data.savedGames.filter(
            save => save.isAutoSave && save.slotId === slot
          );
  ...
  advanceWeek: (gameData) => {
    ...
    // Rastgele ekonomik güncellemeler
    if (gameData.economy) {
  ...
  applyAction: (actionId, gameData) => {
    ...
    switch(actionId) {
      ...
      case 'survey':
        ...
        // Diğer partiler için rastgele değerler (toplamı yaklaşık %100 olacak şekilde)
